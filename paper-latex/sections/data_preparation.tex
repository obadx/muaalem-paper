...
\textbf{Transcription Verification}: We utilized the Tarteel model, which has a 5.7544\% Word Error Rate (WER). To correct potential transcription errors, we leveraged the fact that our input consists of 100\% correct expert recitations to design the \textit{Tasmeea}-inspired algorithm. This algorithm matches the Tarteel transcription to the Quranic text—analogous to a student reciting to a teacher—using fuzzy matching with a moving pointer. If the text matching threshold falls below $85\%$, the samples are referred to human annotators for correction.
