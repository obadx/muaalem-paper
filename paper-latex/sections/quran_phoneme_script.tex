...
Our script comprehensively addresses all three aspects through main two output levels:
\begin{itemize}
    \item \textbf{Phonemes Level}: Represents letters, vowels, and Tajweed rules
    \item \textbf{Sifat Level}: Consists of 10 levels representing articulation attributes for each phoneme
\end{itemize}
...
Example: In table~\ref{tab:examples_with_sifat} shows how our phonetizer works. This example showes the phonetization of word (\arb{أَتُحَٰٓجُّوٓنِّى}) as a row by row: The firt row showes conversion of (أَ) to (ءَ) with its sifat in the row. following the second row. For the forth row showing the madd lazem rule with 6 beats phonetized as 6 alifs (اااااا) same as the sixth row but with damma represented as 6 waw\_madd (ۥۥۥۥۥۥ) And for the fifth row we notice that we converted (ًُجُّ) to (ججُ) as we dissmple shadda. The last row showes the normal madd of yaa with two beats represented as two yadd\_madd (ۦۦ).
...
\begin{table*}[h]
\centering
	\caption{Examples of Uthmani to Phonetic Script Conversion with Sifat Attributes. This example showes the phonetization of word (\arb{أَتُحَٰٓجُّوٓنِّى}) as a row by row: The firt row showes conversion of (\arb{أَ}) to (ءَ) with its sifat in the row. following the second row. For the forth row showing the madd lazem rule with 6 beats phonetized as 6 alifs (\arb{اااااا}) same as the sixth row but with damma represented as 6 waw\_madd (\arb{ۥۥۥۥۥۥ}) And for the fifth row we notice that we converted (ًُ\arb{جُّ}) to (\arb{ججُ}) as we dissmple shadda. The last row showes the normal madd of yaa with two beats represented as two yadd\_madd (\arb{ۦۦ})}
